...
\begin{parts}
  ...
  \end{solution}
  \vfill
  \part How many elements of $\mathbb{F}$ are there in the real interval $[5, 500]$, including the endpoints?
  \begin{solution}
  With base 10 and precision $d=1$, elements of $\mathbb{F}$ have the form $a.b \times 10^n$ where $a \in \{1,...,9\}$ and $b \in \{0,1,...,9\}$.
  
  \textbf{In $[5, 9.9]$:} $5.0, 5.1, ..., 9.9$ $\rightarrow$ 50 elements
  
  \textbf{In $[10, 99]$:} $10, 11, 12, ..., 99$ $\rightarrow$ 90 elements
  
  \textbf{In $[100, 500]$:} $100, 110, 120, ..., 500$ $\rightarrow$ 41 elements
...
\end{parts}
  ...
  \newpage
   Let $a = 5.65$, $b = 7.78$, $c = 3.34$.

  \question[6]

    Suppose $\mathrm{fl}(x)$ is the number in $\mathbb{F}$ obtained by truncating the real number $x$ (i.e., chopping off digits beyond the precision).
Compute the following, showing intermediate steps:

\begin{parts}
  \part $a + (b - c)$
  \begin{solution}
  \textbf{Step 0:} Convert to $\mathbb{F}$: $\mathrm{fl}(a) = 5.6$, $\mathrm{fl}(b) = 7.7$, $\mathrm{fl}(c) = 3.3$
  
  \textbf{Step 1:} Compute $b - c = 7.7 - 3.3 = 4.4$ (already in $\mathbb{F}$)
  ...
  \textbf{Step 1:} Compute $a + b = 5.6 + 7.7 = 13.3$
  
  \textbf{Step 2:} Truncate to $\mathbb{F}$: $\mathrm{fl}(13.3) = 13$
  
  \textbf{Step 3:} Compute $13 - c = 13 - 3.3 = 9.7$ (already in $\mathbb{F}$)
...
\end{parts}
    ...
    Suppose instead $\mathrm{fl}(x)$ is the number in $\mathbb{F}$ \textbf{closest} to the real number $x$ (round half up).
Compute the following, showing intermediate steps:

\begin{parts}
  \part $a + (b - c)$
  \begin{solution}
  \textbf{Step 0:} Convert to $\mathbb{F}$: $\mathrm{fl}(a) = 5.7$, $\mathrm{fl}(b) = 7.8$, $\mathrm{fl}(c) = 3.3$
  
  \textbf{Step 1:} Compute $b - c = 7.8 - 3.3 = 4.5$ (already in $\mathbb{F}$)
  ...
  \textbf{Step 1:} Compute $a + b = 5.7 + 7.8 = 13.5$
  
  \textbf{Step 2:} Round to $\mathbb{F}$: $\mathrm{fl}(13.5) = 14$ (round half up)
  
  \textbf{Step 3:} Compute $14 - c = 14 - 3.3 = 10.7$
