...
Above, I provide practice row reducing a matrix by hand. You can either choose a random matrix to reduce (choosing the dimenions and rank), you can enter a custom matrix (which may be useful when working through homework problems), or you can play the Adventure mode, where you learn new skills and scale up in difficults. 
The par shown is the fewest row operations needed to reach reduced row echelon form from that starting matrix, so it doubles as a rough measure of how much work the system will take.
