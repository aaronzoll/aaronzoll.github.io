...
and whether the system is consistent. Note that when the rank is less than the number of columns, there must be free variables, so there must be infinite solutions.

The matrix dimensions and the rank are yours to set, which is the point. You have the freedom to set the problem's "difficult", and you can play around to see how adjusting the rank changes the underlying structure (and number of computations needs to reduce the matrix). 
